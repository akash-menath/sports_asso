...
export default function Header({ isSidebarOpen, setIsSidebarOpen }: { isSidebarOpen?: boolean; setIsSidebarOpen?: (state: boolean) => void }) {
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-30 bg-white border-b border-gray-200 px-4 py-3 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <img src="/logo.png" alt="KSA Logo" className="h-8 w-auto" />
        </div>
        <button
      ...
      <header className="hidden lg:block bg-gray-50 border-b border-gray-200 px-6 py-3 shrink-0">
        <div className="flex justify-between items-center w-full">
          ...
          <div className="flex items-center gap-4">
            <button
              onClick={() => setIsSidebarOpen?.(!isSidebarOpen)}
              className="h-10 w-10 border border-gray-200 rounded-full flex items-center justify-center text-gray-500 hover:text-gray-700 hover:bg-gray-50 transition-colors focus:outline-none"
            >
